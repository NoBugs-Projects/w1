...
append_test_metrics() {
  ...
  if command -v xargs >/dev/null 2>&1; then
    ...
    manual_tests="$(find "$test_dir" -name "*.java" -type f -print0 2>/dev/null | xargs -0 grep -c "@ManualTest" 2>/dev/null | awk -F: '{sum += $2} END {print sum+0}' 2>/dev/null || echo 0)"
  else
  ...
  if command -v xargs >/dev/null 2>&1; then
    lines="$(find "$test_dir" -type f -name '*.java' -print0 2>/dev/null | xargs -0 cat 2>/dev/null | wc -l 2>/dev/null | tr -d '[:space:]' || echo 0)"
  else
    lines="$(find "$test_dir" -type f -name '*.java' 2>/dev/null -exec cat {} + 2>/dev/null | wc -l 2>/dev/null | tr -d '[:space:]' || echo 0)"
  fi
  lines="$(to_int "$lines")"
